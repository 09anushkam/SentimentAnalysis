...
review_text = df["Text"]
#Initialize the vander sentiment intensity Analyzer
analyzer = SentimentIntensityAnalyzer()

#analyze the sentiment and the subjectivity
sentiment_scores= []
blob_subj = []
...
for sentiment_scores, sentiment_classes in zip(sentiment_scores, sentiment_classes):
    sentiment_scores_by_class[sentiment_classes].append(sentiment_score)
...
plt.ylabel("Count")
plt.title("Score distibution by class")
plt.legend()
st.pyplot(plt)

#Dataframes with sentiment analysis
